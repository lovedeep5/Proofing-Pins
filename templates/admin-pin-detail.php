			...
			<?php if ( $elementor_edit_url ) : ?>
				<div class="pp-elementor-link">
					...
					<a href="<?php echo esc_url( $elementor_edit_url ); ?>" target="_blank" rel="noopener">
						<?php echo esc_html( sprintf( __( 'Open this %s widget in Elementor', 'proofing-pins' ), $elementor_widget_type ) ); ?> &#8599;
					</a>
				</div>
			<?php endif; ?>
			<?php if ( $ai_enabled ) : ?>
				<div class="pp-ai-block" id="pp-ai-block" data-pin-id="<?php echo (int) $pin->ID; ?>" data-status="<?php echo esc_attr( $ai_status ); ?>">
					...
					<div class="pp-ai-body">
						...
						<?php elseif ( $ai_status === 'error' ) : ?>
							<p class="pp-ai-err"><?php echo esc_html( sprintf( __( 'AI error: %s', 'proofing-pins' ), $ai_error ?: 'unknown' ) ); ?></p>
						<?php else :
							...
							<div class="pp-ai-chips">
								...
								<span class="pp-ai-chip pp-ai-chip-risk pp-risk-<?php echo esc_attr( $risk ); ?>"><?php echo esc_html( sprintf( __( 'Risk: %s', 'proofing-pins' ), $risk ) ); ?></span>
							...
							<?php if ( $ai_model_used ) : ?>
								<div class="pp-ai-model"><?php echo esc_html( sprintf( __( 'Generated by %s', 'proofing-pins' ), $ai_model_used ) ); ?></div>
							<?php endif; ?>
						<?php endif; ?>
					</div>
				</div>
			...
			<?php if ( is_array( $change_op ) ) :
				...
				<div class="pp-apply-card" id="pp-apply-card" data-pin-id="<?php echo (int) $pin->ID; ?>">
					<div class="pp-apply-head">
						...
						<?php if ( $applied_at ) : ?>
							<span class="pp-apply-status applied"><?php echo esc_html( sprintf( __( 'Applied %s', 'proofing-pins' ), human_time_diff( strtotime( $applied_at ), current_time( 'timestamp', true ) ) . ' ' . __( 'ago', 'proofing-pins' ) ) ); ?></span>
						<?php endif; ?>
					</div>
